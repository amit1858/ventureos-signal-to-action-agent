...
> **Companion deep-dives:** this is the system-level map. For the multi-agent
> runtime see [Agent Architecture](AGENT_ARCHITECTURE.md); for the trust model
> see [Governance](GOVERNANCE.md); for the post-approval lifecycle see
> [Revenue Execution](REVENUE_EXECUTION.md); for the NVIDIA mapping see
> [NVIDIA Alignment](NVIDIA_ALIGNMENT.md); for the planned voice layer see
...
> **Deep dive:** the six agents, their typed contracts, and the exact scoring and
> confidence formulas are documented in [Agent Architecture](AGENT_ARCHITECTURE.md).
...
| **HubSpot test CRM** | Live demo | 99 demo companies in a *test* portal, pulled via a private-app token. Read for sync; write only after approval. |
...
- **Token safety** — the HubSpot private-app token is read from the environment, sent only as a Bearer
  header, and never logged, returned, or placed into an error message.

> **Deep dive:** the Governance Agent, the human-approval invariant, the Decision
> Ledger, evidence, and BYOK are documented in [Governance](GOVERNANCE.md).
...
- The **HubSpot private-app token lives only on the backend host**, never in the frontend or in Vercel env.
...
baseline), NVIDIA Nemotron / NIM (adapter stub - ready for a live endpoint), OpenAI and Anthropic Claude
...
The product has **two independent model layers** — stating this plainly avoids a
common misread:

| Layer | Code | Used by | Status today |
|---|---|---|---|
| **Reasoning adapters** | `model_adapters/` | The **Communication Agent** inside `/api/recommendations` (draft email / call script) | **Mock by default.** The OpenAI, Claude and NVIDIA NIM adapters are stubs that fall back to the deterministic mock unless `MODEL_PROVIDER` is set server-side. |
| **Decision providers (BYOK)** | `decision_providers/` | The advisory **Comparison / Reasoning overlay** via `/api/decision-providers/*` | **Live** when a user brings a session key (OpenAI / Anthropic / NVIDIA). The key travels in one request, is never stored, and powers the advisory overlay only. |

So when a user connects a BYOK key it lights up the *advisory* reasoning overlay
(the "Executive synthesis" card and "Reasoning Source" strip) — it does **not**
change the deterministic ranking, scores, confidence or governance, and it does
**not** re-route the Communication Agent's drafts. Facts come from the engine; a
provider only narrates.
...
## 15. AI Reasoning Experience & Transparency Layer (Phase 6)
...
## 16. Decision Ledger and action lifecycle
...
- [Agent Architecture](AGENT_ARCHITECTURE.md) — the six agents and their typed contracts
- [Governance](GOVERNANCE.md) — trust model, approval gate, Decision Ledger, BYOK
- [Revenue Execution](REVENUE_EXECUTION.md) — the approved-to-outcome lifecycle
- [NVIDIA Alignment](NVIDIA_ALIGNMENT.md) — mapping onto NIM / Nemotron / NeMo
- [Voice Chief of Staff](VOICE_CHIEF_OF_STAFF.md) — the planned voice layer
- [Product Overview](PRODUCT_OVERVIEW.md) · [Quick Start](QUICK_START.md) · [Roadmap](ROADMAP.md)
- Detailed NVIDIA engineering plan: [nvidia-integration-plan.md](nvidia-integration-plan.md)
