...
// ====================================
// Isolated, self-contained, self-guided tester documentation route. It is NOT
// added to Production navigation and intercepts no Production traffic. A slim
// self-contained header keeps it visually coherent with the shell while showing
// the Tester Guide label.

import Link from "next/link";
import { BookOpenCheck } from "lucide-react";

import { TesterGuide } from "@/components/tester-guide/TesterGuide";
import { GUIDE_META } from "@/lib/tester-guide/content";

export const dynamic = "force-dynamic";

export const metadata = {
  title: "Tester Guide · VentureOS",
  description: GUIDE_META.purpose,
};
  ...
  return (
    <div className="flex min-h-screen flex-col bg-base">
      <header className="sticky top-0 z-30 border-b border-edge bg-base/85 backdrop-blur">
        <div className="mx-auto flex w-full max-w-[1000px] flex-wrap items-center justify-between gap-3 px-5 py-3">
          <Link
            href="/"
          ...
          >
            <div className="relative flex h-9 w-9 items-center justify-center rounded-xl border border-brand/40 bg-gradient-to-br from-brand/25 to-brand/5 transition-colors group-hover:border-brand/70">
              <BookOpenCheck size={18} className="text-brand-bright" />
            </div>
            <div>
              ...
              <p className="text-[11px] leading-tight text-muted">
                Self-guided testing — one signal, one mission, one governed outcome
              </p>
            </div>
          </Link>
          <a href={GUIDE_META.pdfPath} download className="btn btn-ghost text-[12px]">
            Download PDF
          </a>
        </div>
      </header>
